    ...
    **/
    ...
    contract Base1 {
        ...
        modifier doSomethingBefore(uint _a) virtual {
            ...
            _; // leave empty to be fullfilled;
        }
    ...
    contract Identifier is Base1 {
        // do so it will inherit father's constructor
...
/**
    ...
    - transfer():
        ...
        2. the limit of gas in transfer() would be 2300, enough for send ETH, but receiver contract cannot do more complex logic in fallback() or receive()
        3. if some fail occur in transfer(), revert will be run automatally.

    - send():
        1. usage: receiver.send(amount)
        2. the limt of gas in send() would be 2300, enough for send ETH, but receiver's contract cannot do more complex logic in fallback() or receice()
...
(bool,types) is it's returns
    ...
    2. invoked another contract by call is not recommand , 
